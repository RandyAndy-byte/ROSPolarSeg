...
def main(args):
    ...
    # prepare model
    my_BEV_model = BEV_Unet(n_class=len(unique_label), n_height=compression_model, input_batch_norm=True, dropout=0.2,
    ...
    # prepare dataset
    train_PLY_dataset1 = PLY_dataset('../data/paris_lille/Lille2.ply', 0.1, 0.05, label_convert_fun=PLfine2coarse,
                                     return_ref=True)
    train_PLY_dataset2 = PLY_dataset('../data/paris_lille/Lille1.ply', 0.1, 0.05, label_convert_fun=PLfine2coarse,
                                     return_ref=True)
    train_PLY_dataset = torch.utils.data.ConcatDataset([train_PLY_dataset1, train_PLY_dataset2])
    val_PLY_dataset = PLY_dataset('../data/paris_lille/Paris.ply', 0.1, 0.05, label_convert_fun=PLfine2coarse,
                                  return_ref=True)
    if model == 'polar':
        train_dataset = spherical_dataset(train_PLY_dataset, grid_size=grid_size, flip_aug=True, ignore_label=255,
                                          rotate_aug=True, fixed_volume_space=True, \
                                          max_volume_space=[15, np.pi, 12], min_volume_space=[0, -np.pi, -3])
        val_dataset = spherical_dataset(val_PLY_dataset, grid_size=grid_size, ignore_label=255, fixed_volume_space=True,
                                        max_volume_space=[15, np.pi, 12], min_volume_space=[0, -np.pi, -3])
    elif model == 'traditional':
        train_dataset = voxel_dataset(train_PLY_dataset, grid_size=grid_size, flip_aug=True, ignore_label=255,
                                      rotate_aug=True, fixed_volume_space=True, \
                                      max_volume_space=[15, 15, 12], min_volume_space=[-15, -15, -3])
        val_dataset = voxel_dataset(val_PLY_dataset, grid_size=grid_size, ignore_label=255, fixed_volume_space=True,
                                    max_volume_space=[15, 15, 12], min_volume_space=[-15, -15, -3])
    ...
    while True:
        ...
        for i_iter, (_, train_vox_label, train_grid, _, train_pt_fea) in enumerate(train_dataset_loader):
                ...
                with torch.no_grad():
                    for i_iter_val, (_, val_vox_label, val_grid, val_pt_labs, val_pt_fea) in enumerate(
                        ...
                        loss = lovasz_softmax(torch.nn.functional.softmax(predict_labels).detach(), val_label_tensor,
                                              ignore=255) + loss_fun(predict_labels.detach(), val_label_tensor)
            ...
            try:
                ...
                loss = lovasz_softmax(torch.nn.functional.softmax(outputs), point_label_tensor, ignore=255) + loss_fun(
...
if __name__ == '__main__':
    ...
    parser.add_argument('-s', '--grid_size', nargs='+', type=int, default=[320, 320, 32],
                        help='grid size of BEV representation (default: [320,320,32])')
